...
<form method="POST" action="{{ route('admin.collection-points.update', $point) }}" class="border p-3 rounded">
  @csrf
  @method('PUT')
  ...
  <div class="mb-3">
    ...
    <textarea name="address" class="form-control" rows="3" required>{{ old('address', $point->address) }}</textarea>
  </div>

  <div class="mb-3">
    <label class="form-label">Link Peta</label>
    <input name="map_link" class="form-control" value="{{ old('map_link', $point->map_link) }}">
  </div>
